...
import com.example.masterReparateur.dto.Affecter;
...
import com.example.masterReparateur.models.DeliveryType;
...
public class ReservationService {
    ...
    private void sendEmails(String clientEmail, String repairerEmail, Reservation reservation) {
        ...
        for (Livreur livreur : livreurs) {
            mailService.sendMail(new NotificationEmail("Nouvelle reservation à livrer", livreur.getEmail(),
                    "Une nouvelle reservation pour vous à livrer  : http://fixi-fy.com/front/new-reservation/"
                            + reservation.getId()));
        }
        for (SupportTechnique supportTechnique : supportTechniques) {
            mailService.sendMail(
                    new NotificationEmail("Nouvelle reservation est mise en place",
                            ...
                            "Une nouvelle reservation est mise en place: http://fixi-fy.com/front/new-reservation/"
                                    + reservation.getId()));
        }
        mailService.sendMail(new NotificationEmail("Nouvelle reservation pour vous", repairerEmail,
                "Une nouvelle reservation pour vous: http://fixi-fy.com/front/new-reservation/"
                        + reservation.getId()));

        mailService.sendMail(new NotificationEmail("Accusé de reservation", clientEmail,
                "Votre reservation a été créée avec succès: http://fixi-fy.com/front/new-reservation/"
                        + reservation.getId()));
    }
    ...
    private void sendNewDeliveryEmail(Reservation reservation, String invoiceText) {
        ...
        for (Livreur livreur : livreurs) {
            mailService.sendMail(new NotificationEmail("Nouvelle reservation à livrer", livreur.getEmail(),
                    "Une nouvelle reservaton pour vous : http://fixi-fy.com/front/new-reservation/deliver-back/"
                            + reservation.getId()));
        }
    }
    ...
    private void notResolvedEmail(Request request) {
        ...
        for (Admin admin : admins) {
            mailService.sendMail(new NotificationEmail("Reservation non résolue", admin.getEmail(),
                    "Reservation non résolue : http://fixi-fy.com/admin/new-request/"
                            + request.getId()));
        }
    }
