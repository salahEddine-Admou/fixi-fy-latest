...
public interface ReservationRepo extends JpaRepository<Reservation, Long> {
    ...
    @Query("SELECT rep.repairer.username as username,rep.repairer.imageProfile as imageProfile, COUNT(r.id) as reservationCount "
            +
            "FROM Reservation r " +
            "JOIN r.reparation rep " +
            "GROUP BY rep.repairer.id " +
    ...
    @Query("SELECT c.username AS name, c.imageProfile AS imageProfile, COUNT(r.id) AS reservationCount " +
            "FROM Reservation r " +
            "JOIN r.client c " +
            "WHERE r.reparation.repairer.username = :username " +
            "GROUP BY c.id " +
            "ORDER BY reservationCount DESC")
    ...
    @Query("SELECT r.reparation.problem.name as problem, COUNT(r) as reservationCount " +
        "FROM Reservation r " +
        "JOIN r.reparation rep " +
        "WHERE rep.repairer.username = :username " +
        "GROUP BY r.reparation.problem")
    ...
    @Query("SELECT r.reparation.problem.categoryModel.name as category, COUNT(r) as reservationCount " +
        "FROM Reservation r " +
        "JOIN r.reparation rep " +
        "GROUP BY r.reparation.problem.categoryModel.name")
